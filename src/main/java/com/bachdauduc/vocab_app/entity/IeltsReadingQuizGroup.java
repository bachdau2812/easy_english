package com.bachdauduc.vocab_app.entity;

import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.Id;
import jakarta.persistence.Table;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Entity
@Table(name = "ielts_reading_quiz_groups")
public class IeltsReadingQuizGroup {
    @Id
    @Column(name = "id", length = 36, nullable = false)
    private String id;

    @Column(name = "reading_source_id", length = 36, nullable = false)
    private String readingSourceId;

    @Column(name = "quiz_type", length = 50, nullable = false)
    private String quizType;

    @Column(name = "group_order", nullable = false)
    private Integer groupOrder;

    @Column(name = "instruction", columnDefinition = "LONGTEXT")
    private String instruction;

    @Column(name = "question_number_start")
    private Integer questionNumberStart;

    @Column(name = "question_number_end")
    private Integer questionNumberEnd;

    @Column(name = "word_limit")
    private String wordLimit;

    @Column(name = "source_paragraph_id", columnDefinition = "LONGTEXT")
    private String sourceParagraphId;

    @Column(name = "shared_options", columnDefinition = "LONGTEXT")
    private String sharedOptions;
}
